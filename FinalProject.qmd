...
df <- df |>
  mutate(logit_pred = predict(fit, type = "link"))  # predicted log-odds

ggplot(df, aes(x = log2_glutwa, y = logit_pred)) +
  geom_point(alpha = 0.4) +
  geom_smooth(method = "loess", se = FALSE, color = "blue") +
  labs(
    title = "Linearity in the log-odds for log₂(glutwa)",
    x = "log₂(glutwa)",
    y = "Predicted log-odds of delayed graft function"
  ) +
  theme_minimal()
```
