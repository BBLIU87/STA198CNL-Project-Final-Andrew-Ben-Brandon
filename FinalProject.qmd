...
df <- read.csv("DonorHyperglycemia.csv") |>
  mutate(
    delayed_fn    = factor(delayed_fn, levels = c(0,1), labels = c("No","Yes")),
    hyperglycemia = factor(if_else(glutwa > 180, 1, 0), levels = c(0,1),
...
df_mod <- df |> drop_na(all_of(vars))

fit <- glm(delayed_fn ~ ., data = df_mod, family = binomial)
summary(fit)

OR <- exp(coef(fit))
CI <- exp(confint(fit))

or_table <- round(cbind(OR, CI), 3)
or_table
