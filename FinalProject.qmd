...
## AI Statement:
https://chatgpt.com/share/69322b7d-de48-800c-9044-9385d96cdddf
AI was used to justify using log2 transformation on glucose variables to make
them not violate linearity on log odds and to download additional packages.
...
The two assumptions for a logistic regression model are independence and linearity in the log-odds. The independence assumption is satisfied, as each data point corresponds to one organ transplant and there are no repeat organ transplants in the data. 
The assumption of linearity in the log-odds is satisfied through the log-tranformation of the variables. To assess the distributional properties of the glucose predictors (glutwa, glurange, glusd), we evaluated right-skewness using numerical diagnostics. Numerical skewness statistics were computed using the e1071 package in R, and all three glucose variables showed positive skewness values greater than 1, indicating substantial right-skewed distributions.
Because logistic regression assumes a linear relationship between each continuous predictor and the log-odds of the outcome, the presence of right-skewness can potentially introduce problems that violate this assumption. To address this, we applied a log₂ transformation to each glucose variable. The log₂ transformation reduces skewness by compressing the upper tail and stabilizing variance, which helps linearize the predictor log-odds relationship. Additionally, the log₂ scale provides a clinically interpretable effect size, where a one-unit increase corresponds to a doubling in the glucose value.
Together, the skewness diagnostics and the theoretical considerations of the logistic model justified the use of the log₂ transformation to improve model fit and interpretability.
